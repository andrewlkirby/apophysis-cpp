...
/* Vendored from apophysis-7x-patched/src/Plugin/apoplugin.h for the C++
   ...
   - DLLIMPORT no longer expands to __declspec(dllexport): these are
     statically linked now, never loaded as a DLL, and dllexport would
     just add ~1000 pointless entries to the final .exe's export table.
   - NO_ASM is forced on: the original's MSVC branch below uses inline
     __asm blocks, which x64 MSVC does not support at all (inline asm was
     removed for x64) - forcing the portable sin()/cos() fallback avoids a
     hard compile error.
   - OFFSET/INT_LVALUE/DOUBLE_LVALUE previously round-tripped a real
     pointer through a 32-bit `int` (`(int) &ptr->field`, later cast back
     to a pointer). On 64-bit, addresses routinely exceed 4GB, so this
     truncated the address and reinterpreted garbage as a pointer - a
     genuine crash/corruption bug, not just a narrowing warning. Fixed by
     using `char*`-based pointer arithmetic instead.
*/

#ifndef _APOPLUGIN_H_
#define _APOPLUGIN_H_

// M_PI and friends (see the comment block below) are only defined by
// MSVC's math.h when this is set before including it - unlike glibc's
// math.h, which the original was written against and defines them
// unconditionally. Must come before <math.h>.
#ifndef _USE_MATH_DEFINES
#define _USE_MATH_DEFINES
#endif

// <malloc.h> deliberately not included: this header only uses calloc/free
// (see below), both already declared by <stdlib.h> - and <malloc.h> exists
// on Windows/glibc but not macOS/BSD, so pulling it in for no reason would
// break the Clang/macOS build.
#include <math.h>
#include <time.h>
#include <string.h>
#include <limits.h>
...
#define DLLIMPORT /* statically linked - see file header comment above */

// Force the portable sin()/cos() fallback below - see file header comment.
#ifndef NO_ASM
#define NO_ASM 1
#endif
...
#define OFFSET(name) ((int) (ptrdiff_t) &(((Variables*) 0)->name))

#define INT_LVALUE(varptr, info)    ( *((int*)    (((char*) &((varptr)->var)) + (info).offset)))
#define DOUBLE_LVALUE(varptr, info) ( *((double*) (((char*) &((varptr)->var)) + (info).offset)))
...
// The original cast a plain double/int literal directly to the IntOrDouble
// union type (`(IntOrDouble) (def)`) - a GNU C "cast to union" extension
// that selects the member matching the source expression's type. That's
// not standard C and MSVC's compiler rejects it outright ("cannot convert
// from 'double' to 'IntOrDouble'"). A standalone compound-literal spelling
// (`(IntOrDouble){.d = ...}`) isn't accepted as a compile-time constant by
// MSVC's C compiler here either (these initialize file-scope VarInfo[]
// arrays). Since each macro already knows statically which member it
// means (REAL-flavored macros always want .d, INTEGER-flavored ones
// always want .i), and each of these is itself a nested initializer for
// an IntOrDouble-typed struct field (not a standalone expression), plain
// nested designated-initializer braces - ordinary aggregate
// initialization, no compound-literal syntax involved - compile cleanly
// as a real constant everywhere. Same resulting union value.
#define VAR_REAL(nm, def) { #nm, REAL, OFFSET(nm), {.d = -DBL_MAX}, {.d = DBL_MAX}, {.d = (def)} }
#define VAR_INTEGER(nm, def) { #nm, INTEGER, OFFSET(nm), {.i = INT_MIN}, {.i = INT_MAX}, {.i = (def)} }
#define VAR_REAL_RANGE(nm, min, max, def) {#nm, REAL, OFFSET(nm), {.d = (min)}, {.d = (max)}, {.d = (def)} }
#define VAR_INTEGER_RANGE(nm, min, max, def) {#nm, INTEGER, OFFSET(nm), {.i = (min)}, {.i = (max)}, {.i = (def)} }
#define VAR_INTEGER_NONZERO(nm, def) {#nm, INTEGER_NONZERO, OFFSET(nm), {.i = INT_MIN}, {.i = INT_MAX}, {.i = (def)} }
#define VAR_REAL_CYCLE(nm, min, max, def) {#nm, REAL_CYCLIC, OFFSET(nm), {.d = (min)}, {.d = (max)}, {.d = (def)} }
...
// static inline (not plain inline): this header is #included, as real C, by
// ~28 separate plugin wrapper translation units (see src/core/plugins/) -
// under C99/C11 inline semantics a plain `inline` function with no matching
// external definition anywhere is only a *candidate* for inlining, and
// GCC/Clang can legitimately emit an external reference to a
// never-defined symbol, causing "undefined reference" link errors (MSVC's
// C inline handling is more lenient and doesn't hit this). `static inline`
// gives every translation unit its own private definition, sidestepping
// the issue entirely - harmless duplication for functions this small.
/* Calculate both the sine and cosine of an angle theta (in radians). */
static inline void fsincos(double theta, double* s, double* c)
{
...
static inline void sinhcosh(double theta, double* sh, double* ch)
{
