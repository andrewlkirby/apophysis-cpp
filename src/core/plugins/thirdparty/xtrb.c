...
/* Hoisted from GCC nested-function definitions inside PluginVarCalc in the
   original (GNU C nested functions aren't supported by MSVC). Unlike the
   nested-function hoists in other plugins in this tree, these three
   actually used their enclosing PluginVarCalc's `vp` parameter via the
   VAR(...)/VVAR macros (GCC nested functions get implicit access to the
   enclosing scope - that's a closure, not just sugar). Fixed by adding an
   explicit `Variation* vp` parameter, which VAR(...)'s expansion
   (`vp->var.name`, see apoplugin.h) then resolves to exactly as before;
   call sites below pass `vp` explicitly. Otherwise byte-identical. */
static void DirectTrilinear(Variation* vp, double x, double y, double* Al, double* Be, double* Ga)
{
...
static void InverseTrilinear(Variation* vp, double Al, double Be, double* x, double* y)
{
...
static void Hex(Variation* vp, double Al, double Be, double Ga, double* Al1, double* Be1) // it's necessary
