...
/* Hoisted from GCC nested-function definitions inside PluginVarCalc in the
   ...
   change: none of these took closure state from PluginVarCalc's locals,
   only their own parameters (hits_triangle calls dot, so hoisted in the
   same already-dependency-respecting order as the original). */
...
static int hits_rect(double2 tl, double2 br, double2 p) {
	return (p.x >= tl.x && p.y >= tl.y && p.x <= br.x && p.y <= br.y);
}
static int hits_triangle(double2 a, double2 b, double2 c, double2 p, double* u, double* v) {
...
static int hits_square_around_origin(double a, double2 p) {
	return (fabs(p.x) <= a && fabs(p.y) <= a);
}
static int hits_circle_around_origin(double radius, double2 p, double* r) {
