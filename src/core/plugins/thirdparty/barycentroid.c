...
// original (`inline double sgn(double v) {...}` declared mid-function) -
// GNU C nested functions aren't supported by any other C compiler,
// including MSVC. No behavior change: this took no closure state from
// PluginVarCalc's locals, only its own parameter.
static double sgn(double v) { return v < 0 ? -1 : v > 0 ? 1 : 0; }
