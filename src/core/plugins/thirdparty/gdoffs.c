...
/* Hoisted from GCC nested-function definitions inside PluginVarCalc in the
   original (GNU C nested functions aren't supported by MSVC). No behavior
   change: none of these took closure state, only their own parameters
   (fscl calls fclp and fosc calls fscl, so hoisted in the same
   already-dependency-respecting order as the original). fcip is unused by
   PluginVarCalc's body in the original too - kept as-is, just relocated. */
static double fcip(double a){return ((a<0)?-((int)(fabs(a))+1):0)+((a>1)?((int)(a)):0);}
static double fclp(double a){return ((a<0)?-(fmod(fabs(a),1)):fmod(fabs(a),1));}
static double fscl(double a){return fclp((a+1)/2); }
static double fosc(double p, double a){return fscl(-1*cos(p*a*M_2PI));}
static double flip(double a, double b, double c){return (c*(b-a)+a);}
