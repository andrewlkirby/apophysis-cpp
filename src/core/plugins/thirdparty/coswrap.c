...
static double flerp(double a, double b, double p) { return (a+(b-a)*p); }
static double fabsmod(double fintp) { double dummy; return modf(fintp, &dummy); }
static double fosc(double p, double amp, double ph) { return 0.5-cos(p*amp+ph)*0.5; }
static double foscn(double p, double ph) { return 0.5-cos(p+ph)*0.5; }
