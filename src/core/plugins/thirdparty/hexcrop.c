...
/* Hoisted from a GCC nested-function definition inside PluginVarCalc in the
   original (GNU C nested functions aren't supported by MSVC). No behavior
   change: its local `px`/`py` locals shadow but never reach outward into
   PluginVarCalc's own px/py - they're always derived solely from this
   function's own parameters (Px, Py, Ox, Oy), so hoisting to file scope
   with the same parameter list is exactly equivalent. */
static void fbarycht(
